...
function SpriteAssetLoader.splitIdentifier(full_identifier, exact)
    local _, _, reverse_frame, reverse_identifier = string.find(string.reverse(full_identifier), "^(%d+)_?([^/]+.*)")
    if not (reverse_frame and reverse_identifier) then
        if exact then return full_identifier end
        return full_identifier, 1
    end
    local split_frame = math.floor(assert(tonumber(string.reverse(reverse_frame))))
    if split_frame < 1 then
        return full_identifier
    end
    return string.reverse(reverse_identifier), split_frame
end

function SpriteAssetLoader:beginLoad(file, queue)
    -- Split an asset id like `"idle_01"` into `"idle", "01"`
    local identifier, split_frame = SpriteAssetLoader.splitIdentifier(file.identifier, true)
    ...
    task.numbered = task.numbered or split_frame ~= nil
...
function SpriteAssetLoader:load(asset_id, task)
    ...
    for _, frame_data in ipairs(task.files) do
        ...
        local success, image_data = pcall(love.image.newImageData, frame_data.path)
        if not success then
            error(string.format("Image \"%s\" is invalid or corrupted!", frame_data.path))
        end
...
function SpriteAssetLoader:apply(asset_id, output)
    ...
    for i, exact_id in pairs(output.frame_ids) do
